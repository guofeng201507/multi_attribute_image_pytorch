...
class Config(object):
    def __init__(self):
        ...
        parser.add_argument('--model_weight_file', type=str,
                            default='./exp/deepmar_resnet50/peta/partition0/run1/model/ckpt_epoch100.pth')
        ...
        if args.dataset in datasets:
            with open(datasets[args.dataset], 'rb') as file:
...
for idx in range(len(cfg.att_list)):
    ...
    print('%s: %.5f' % (cfg.att_list[idx], sigmoid_score))
...
for idx in range(len(cfg.att_list)):
    if score[0, idx] >= 0:
        ...
        txt = '%s: %.5f' % (cfg.att_list[idx], sigmoid_score)
